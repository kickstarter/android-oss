...
import com.kickstarter.ui.views.compose.checkout.CollectionPlan
...
class CollectionPlanTest : KSRobolectricTestCase() {
    ...
    private val pledgeInFullOption = composeTestRule.onNodeWithTag(CollectionPlanTestTags.OPTION_PLEDGE_IN_FULL.name)
    private val pledgeOverTimeOption = composeTestRule.onNodeWithTag(CollectionPlanTestTags.OPTION_PLEDGE_OVER_TIME.name)
    private val descriptionText = composeTestRule.onNodeWithTag(CollectionPlanTestTags.DESCRIPTION_TEXT.name)
    private val badgeText = composeTestRule.onNodeWithTag(CollectionPlanTestTags.BADGE_TEXT.name)
    private val expandedText = composeTestRule.onNodeWithTag(CollectionPlanTestTags.EXPANDED_TEXT.name)
    ...
    fun `test screen init with eligible pledge in full`() {
        ...
        val description = context.getString(R.string.fpo_you_will_be_charged_for_your_pledge_over_four_payments_at_no_extra_cost)
    ...
    fun `test terms and expanded text when pledge over time is selected`() {
        val expandedTextValue = context.getString(R.string.fpo_the_first_charge_will_be_24_hours_after_the_project_ends_successfully)
