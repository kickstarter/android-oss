...
class UpdateViewHolder(private val view: View, val delegate: Delegate?) : KSViewHolder(view) {
    ...
    init {
        ...
        this.viewModel.outputs.likesCount()
                .compose(bindToLifecycle())
                .compose(observeForUI())
                .subscribe { this.view.update_likes_count.text = it.toString() }
    ...
    private fun setCommentsCount(commentsCount: Int) {
        ...
        this.view.update_comments_count.contentDescription = this.ksString.format("comments_count_comments", commentsCount,
                "comments_count", NumberUtils.format(commentsCount))
    }
