...
public class TokenListenerService extends InstanceIDListenerService {
   ...
   * InstanceID service might periodically request that we swipeRefresh a token every 6 months, or
   * it could force a swipeRefresh if a token is invalidated (e.g. security issues, device information
   * is no longer valid. When a token swipeRefresh event is received, we create a new register intent
