...
class SegmentTest : KSRobolectricTestCase() {
    ...
    private fun assertProjectProperties(project: Project) {
        ...
        assertEquals(project.deadline()?.millis?.let { (it / 1000).toString() }, expectedProperties["project_deadline"])
        ...
        assertEquals(project.launchedAt()?.millis?.let { (it / 1000).toString() }, expectedProperties["project_launched_at"])
